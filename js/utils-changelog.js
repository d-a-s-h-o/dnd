...
class UtilsChangelog {
  static renderChangelog(changelog, $wrp) {
    ...
    changelog.forEach((it, i) => {
      ...
      const hLevel =
        vMajor !== lastMajorVersion
          ? "2"
          : vMinor !== lastMinorVersion
          ? "3"
          : "4";
      ...
      const titlePart = it.title
        ? `, &quot;<span ${
            it.titleAlt
              ? `class="help" title="AKA &quot;${it.titleAlt.escapeQuotes()}&quot; Edition"`
              : ""
          }>${it.title.escapeQuotes()}</span>&quot; Edition`
        : "";
      $wrp.prepend(
        `<div class="flex-col" id="v${
          it.ver
        }">\n\t\t\t\t<div class="split-v-center">\n\t\t\t\t\t<h${hLevel} class="bold">v${
          isLast ? `<a href="https://github.com/thenullnetwork/dnd/releases/latest">` : ""
        }${it.ver}${
          isLast ? `</a>` : ""
