class RenderObjects{static $getRenderedObject(obj){const renderer=Renderer.get().setFirstSection(true);const renderStack=[];if(obj.entries)renderer.recursiveRender({entries:obj.entries},renderStack,{depth:2});if(obj.actionEntries)renderer.recursiveRender({entries:obj.actionEntries},renderStack,{depth:2});const hasToken=obj.tokenUrl||obj.hasToken;const extraThClasses=hasToken?["objs__name--token"]:null;return $$`
			${Renderer.utils.getBorderTr()}
			${Renderer.utils.getExcludedTr(obj,"object")}
			${Renderer.utils.getNameTr(obj,{extraThClasses:extraThClasses,page:UrlUtil.PG_OBJECTS})}
