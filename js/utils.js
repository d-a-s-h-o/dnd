...
VERSION_NUMBER = /* DNDZ_VERSION__OPEN */"4.0.0"/* DNDZ_VERSION__CLOSE */;
DEPLOYED_STATIC_ROOT = ""; // "https://cdn.dndz.dev/"; // FIXME re-enable this when we have a CDN again
...
VeCt = {
	...
	LOC_ORIGIN_CANCER: "https://dndz.dev",
...
UrlUtil = {
	...
	/**
	 * All internal URL construction should pass through here, to ensure `cdn.dndz.dev` is used when required.
	 *
	 * @param href the link
	 */
	link (href) {
	...
	window.addEventListener("keypress", (e) => {
		if (EventUtil.noModifierKeys(e) && typeof d20 === "undefined") {
			if (e.key === "#") {
				...
				window.prompt("Copy to clipboard: Ctrl+C, Enter", `https://dndz.dev/${spl[spl.length - 1]}`);
			}
		}
	});
}
...
DataUtil = {
	...
	class: {
		...
		async pGetDereferencedClassData (cls) {
			...
			for (const classFeatureRef of (cls.classFeatures || [])) {
				...
				if (source === SRC_DNDZ_TMP) continue; // Skip over temp/nonexistent links
	...
	brew: {
		_getCleanUrlRoot (urlRoot) {
			...
			} else urlRoot = `https://raw.githubusercontent.com/d-a-s-h-o/dnd-homebrew/master/`;
			return urlRoot;
		},
...
BrewUtil = {
	...
	async _pRenderBrewScreen ($appendTo, opts) {
		...
		const $wrpBtns = $$`<div class="ve-flex-vh-center no-shrink mobile__ve-flex-col">
			...
			<div class="ve-flex-v-center">
				<a href="https://github.com/d-a-s-h-o/dnd-homebrew" class="ve-flex-v-center" target="_blank" rel="noopener noreferrer"><button class="btn btn-default btn-sm">Browse Source Repository</button></a>
				${$btnDelAll}
			</div>
		</div>`;
	...
	async _pHandleClickBtnGet (opts) {
		...
		Contributions are welcome; see the <a href="https://github.com/d-a-s-h-o/dnd-homebrew/blob/master/README.md" target="_blank" rel="noopener noreferrer">README</a>, or stop by our <a href="https://discord.com/invite/yM7YGrT" target="_blank" rel="noopener noreferrer">Discord</a>.</i></div>
		<hr class="hr-1">
...
_Donate = {
	...
	init () {
		if (IS_DEPLOYED) {
			DataUtil.loadJSON(`https://donate.dndz.dev`).then(dosh => {
